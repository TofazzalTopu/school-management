...
jQuery(function ($) {
            ...
            success: function (data) {
                ...
                if(data.isError==true){
                    ...
                    $.each(data.sectionList, function (key, value) {
                        $('#list-table').append('<tr><td>' + value.name +'</td><td><input type="hidden" name="sectionIds" value="'+value.id+'"><input type="number" name="start'+value.id+'" required></td></tr>');
                    });
    ...
    $('#batchPromotionForm').on('submit', function(e) { //use on if jQuery 1.7+
        if( confirm('Are you sure to promote? This action can not undone') ){
            var formAction = "${g.createLink(controller: 'student', action: 'oddEvenPromotion')}";
