...
<script>
    var workingYear, viewHrefUrl, viewHistoryHrefUrl, academicYear, className, sectionName, printParam;
    jQuery(function ($) {
        viewHrefUrl = "${g.createLink(controller: 'registration',action: 'addDetails')}/";
        viewHistoryHrefUrl = "${g.createLink(controller: 'registration',action: 'showHistory')}/";
        ...
        var table = $('#list-table').dataTable({
            ...
            "iDisplayLength": 25,
            "aaSorting": [2, 'desc'],
            "sAjaxSource": "${g.createLink(controller: 'student',action: 'list')}?workingYear=${workingYear?.key}",
    ...
    function getActionButtons(nRow, aData) {
        ...
        actionButtons += '</a></span>';
        actionButtons += '<span class="col-md-3 no-padding"><a target="_blank" href="' + viewHistoryHrefUrl + aData.registrationId + '" class="view-reference" title="History">';
        actionButtons += '<span class="fa fa-history"></span>';
        actionButtons += '</a></span>';
